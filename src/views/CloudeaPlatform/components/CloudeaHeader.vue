...
export default {
    data() {
        return {
            ...
            // css管理
            styleManage: {
                searchFocusBar: 'border-radius: 8px;'
            },
            // 左栏 标题+链接
            leftEntryLinkArr: [
                {
    ...
    methods: {
        ...
        HandleSearchFocus() {
            ...
            this.classManage.searchFocus.push('is-focus')
            this.styleManage.searchFocusBar = 'border-radius: 8px 8px 0px 0px;'
        },
        HandleSearchBlur() {
            this.searchFocus = false;
            this.classManage.searchFocus.pop('is-focus')
            this.styleManage.searchFocusBar = 'border-radius: 8px;'
        }
    },
    computed: {
        SearchPanelStyle() {
            return this.searchFocus ? '' : 'display:none'
        }
    }
}
</script>
...
<template>
    <div class="cloudeaMainHeader" style="height: 64px;">
        <div class="cloudea-header fixed-header">
            <div class="cloudea-header__bar mini-header">
                <ul class="left-entry">
                    <li>
                        <a href="#" class="left-entry__title">
                            <svg t="1626764977164" viewBox="0 0 2240 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                p-id="4404" xmlns:xlink="http://www.w3.org/1999/xlink" width="140" height="64"
                                class="mini-header__logo">
                                <path
                    ...
                    <li v-for="(item, index) in leftEntryLinkArr" :key="index" class="v-popover-wrap">
                        <a :href="item.link" target="_blank" class="default-entry">
                            <span>{{ item.title }}</span>
                        </a>
                    </li>
                </ul>
                <div class="center-search-container">
                    <div :class="'center-search__bar ' + classManage.searchFocus">
                        <!-- 搜索框 -->
                        <form id="nav-searchform" :class="classManage.searchFocus" :style="styleManage.searchFocusBar">
                            <div class="nav-search-content">
                                <input @focus="HandleSearchFocus()" @blur="HandleSearchBlur()" class="nav-search-input" type="text"
                                    v-model="searchText" autocomplete="off" accesskey="s" maxlength="100" x-webkit-speech=""
                                    x-webkit-grammar="builtin:translate" placeholder="搜索" title="搜索">
                                <div :class="classManage.searchClean">
                            ...
                            <div @click="Search" class="nav-search-btn">
                        ...
                        <!-- 搜索历史/推荐 -->
                        <div class="search-panel" :style="SearchPanelStyle">
                            <div class="history" style="max-width: 498px;">
                                <div class="header">
                                ...
                                <div class="histories-wrap" style="max-height: 92px;">
                                    <div class="histories">
                                        <div v-for="(a, index) in searchHistory" :key="index" class="history-item"
                                            tabindex="0">
                ...
                0.004093s12.493545 32.729425 0.002047 45.220924L557.225017 512 710.491727 665.266709z"></path>
                                    ...
                                    <div class="history-fold-wrap">
                                        <div class="fold-text">展开更多</div><svg class="fold-icon fold" viewBox="0 0 12 12">
              ...
              C1.07618 4.46671 1.07618 4.78329 1.27145 4.97855L5.46967 9.17678Z"></path>
                                ...
                                <div class="trendings-double">
                                    <div class="trendings-col" style="max-width: 244px;">
                                        <div class="trending-item" tabindex="0">
                                            ...
                                            <div class="trending-text">12306回应二等座无座同价</div>
                                        </div>
                                        <!-- ... -->
                                    </div>
                                    <div class="trendings-col" style="max-width: 244px;">
                                        <div class="trending-item" tabindex="0">
                                            ...
                                            <div class="trending-text">妈妈是超人却追不上时间</div>
                                        </div>
                                        <!-- ... -->
                                    </div>
                                </div>
                ...
                <ul class="right-entry">
                    <li v-for="(item, index) in rightEntryLinkArr" :key="index" class="v-popover-wrap">
                        <a :href="item.link" target="_blank" class="default-entry">
                            <span>{{ item.title }}</span>
                        </a>
                    </li>
...
.cloudea-header .cloudea-header__bar {
    position: absolute;
    top: 0;
    z-index: 1002;
    display: flex;
...
.fixed-header .cloudea-header__bar {
    position: fixed;
    top: 0
}
...
.mini-header .left-entry {
    margin-right: 5px
}

.cloudea-header .left-entry__title .mini-header__logo {
    color: #fff;
    margin-right: 25px;
    width: 60px;
    cursor: pointer;
...
.cloudea-header .left-entry__title {
    display: flex;
    align-items: center;
    justify-content: space-between;
    margin-right: 20px;
...
.cloudea-header .left-entry__title .mini-header__title {
    display: flex;
    align-items: center;
    text-align: center;
    font-size: 14px;
...
.cloudea-header .left-entry .default-entry {
...
/* 搜索框 */
.cloudea-header .center-search-container {
    flex: 1 auto;
    height: 38px;
}

.mini-header .center-search-container {
    margin: 0 10px;
}

.cloudea-header .center-search-container .center-search__bar {
    position: relative;
    margin: 0 auto;
    min-width: 181px;
    max-width: 500px;
...
.cloudea-header .center-search-container .center-search__bar.is-focus {
    box-shadow: 0 0 30px #0000001a;
    border-radius: 8px;
}

.cloudea-header .center-search-container .center-search__bar #nav-searchform {
    display: flex;
    align-items: center;
...
.cloudea-header .center-search-container .center-search__bar #nav-searchform:hover {
    border: 1px solid #E3E5E7;
    background: #fff;
}
...
.cloudea-header .center-search-container .center-search__bar #nav-searchform.is-actived .nav-search-content,
.cloudea-header .center-search-container .center-search__bar #nav-searchform.is-focus .nav-search-content {
    background-color: #E3E5E7;
}

.cloudea-header .center-search-container .center-search__bar .nav-search-content {
    display: flex;
    align-items: center;
    justify-content: space-between;
    position: relative;
...
.cloudea-header .center-search-container .center-search__bar .nav-search-content .nav-search-input {
    flex: 1;
    overflow: hidden;
    padding-right: 8px;
    border: none;
...
.cloudea-header .center-search-container .center-search__bar .nav-search-content .nav-search-clean {
    width: 16px;
    height: 16px;
    right: 10px;
    cursor: pointer;
...
.cloudea-header .center-search-container .center-search__bar .nav-search-content .nav-search-clean svg {
    position: absolute;
    color: #C9CCD0;
}

.cloudea-header .center-search-container .center-search__bar .nav-search-content .nav-search-clean.has-keyword {
    visibility: inherit;
}

.cloudea-header .center-search-container .center-search__bar .nav-search-btn {
    position: absolute;
    top: 3px;
    right: 7px;
    display: flex;
...
.cloudea-header .center-search-container .center-search__bar .nav-search-btn:hover {
    background-color: #E3E5E7;
}

/* 右栏 */
.cloudea-header .right-entry {
    display: flex;
    align-items: center;
    margin-left: 50px;
}

.cloudea-header .right-entry .default-entry {
    display: inline-block;
}
...
.cloudea-header .header .title {
    height: 24px;
    font-size: 16px;
    line-height: 24px;
}

.cloudea-header .header .clear {
    font-size: 12px;
    line-height: 15px;
    height: 15px;
    color: #9499A0;
...
/* 搜索框面板 - 历史 */
.cloudea-header .histories-wrap {
    padding: 0 16px;
    overflow: hidden;
}

.cloudea-header .histories {
    display: flex;
    flex-wrap: wrap;
    margin-top: 12px;
    margin-right: -10px;
...
.cloudea-header .histories .history-item {
    position: relative;
...
.cloudea-header .histories .history-item .history-text {
    white-space: nowrap;
    overflow: hidden;
...
.cloudea-header .histories .history-item .close {
    display: none;
    box-sizing: border-box;
    position: absolute;
    width: 16px;
...
.cloudea-header .histories .history-item .close svg {
    width: 14px;
    height: 14px;
    fill: #ccc;
}

.cloudea-header .histories .history-item:hover .close {
    display: inherit;
}

/* 搜索框面板 - 历史 - 展开 */
.cloudea-header .history-fold-wrap {
    display: flex;
    align-items: center;
    justify-content: center;
    width: 64px;
...
.cloudea-header .fold-icon.fold {
    transform: rotate(0);
}

/* 搜索框面板 - 推荐 */
.cloudea-header .trendings-double {
    display: flex;
}

.cloudea-header .trendings-double .trendings-col:first-child {
    margin-right: 10px;
}

.cloudea-header .trendings-double .trendings-col {
    flex: 1;
}

.cloudea-header .trending-item {
    box-sizing: border-box;
    height: 38px;
    display: flex;
    align-items: center;
...
.cloudea-header .trending-item .trendings-rank {
    width: 15px;
    min-width: 15px;
    height: 17px;
    line-height: 17px;
...
.cloudea-header .trending-item .trending-text {
    font-size: 14px;
    line-height: 17px;
    height: 17px;
    margin-right: 6px;
