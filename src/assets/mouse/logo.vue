...
<script setup lang="ts">
  ...
  const logos = reactive([
    { label: 'kazimierz', url: 'https://i.postimg.cc/FRJ7WZp2/logo-kazimierz.png' },
    { label: 'rhine', url: 'https://i.postimg.cc/wxL1Dw7N/logo-rhine.png' },
    { label: 'rhodes', url: 'https://i.postimg.cc/L5thR8rD/logo-rhodes.png' },
    { label: 'victoria', url: 'https://i.postimg.cc/ZKL0M0PK/logo-victoria.png' },
    { label: 'yan', url: 'https://i.postimg.cc/Nft5DRHz/logo-yan.png' },
    { label: '1', url: '/src/assets/images/logos/island.png' },
  ...
  class LogoImg {
    ...
    constructor(src: string, name: string) {
      ...
      img.onload = () => {
        ...
        const tmp_ctx = tmp_canvas.getContext('2d') // 设置为2D

        const imgW = width // 动态获取宽度
        const imgH = ~~(width * (img.height / img.width)) // 动态获取高度（取整）
        ...
        tmp_ctx?.clearRect(0, 0, width, height) // 清空原图

        // 筛选像素点，将符合条件的像素点生成为粒子
        for (let y = 0; y < imgH; y += 5) {
          ...
          for (let x = 0; x < imgW; x += 5) {
            ...
            // 筛选条件为透明度
            if (sum >= 100) {
              const particle = new Particle(x, y, animateTime, [255, 255, 255, a])
